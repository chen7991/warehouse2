...
public class AppFileUtils {
	//文件上传的保存路径
	public static  String UPLOADPATH="D:/upload/";//默认值
	...
	static {
		...
		String property = properties.getProperty("filepath");//得到properties的地址
		if(null!=property) {
			UPLOADPATH=property;
		}
	}
...
//			header.setContentDispositionFormData("attachment", "123.jpg");
	...
	public static String renameFile(String goodsimg) {
		File file=new File(UPLOADPATH, goodsimg);
		String replace = goodsimg.replace("_picture", "");
		if(file.exists()) {
			file.renameTo(new File(UPLOADPATH, replace));
		}
		return replace;
	}
